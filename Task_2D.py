...
def read_csv(csv_name):
    ...
    with open('lat_long.csv', mode ='r') as file:
...
def tracker(ar_id, lat_lon):
    ...
    with open('lat_long.csv', mode ='r') as file:
        ...
        for lines in csvFile:
            ...
            if(key==ar_id):
                coordinate = []
                coordinate.append((lines[1]))
                coordinate.append((lines[2]))
                loc1 = coordinate
                with open('live_data.csv', 'w') as csvfile:  
                    csvwriter = csv.writer(csvfile)
                    csvwriter.writerow(loc1)
